...
public class FoodDao {
	...
	public List<String> getVertici(Double calorie){
		String sql = "SELECT DISTINCT(p.portion_display_name) AS vert "
				+ "FROM `portion` AS p "
				+ "WHERE p.calories<? "
	...
	public List<Adiacenza> getArchi(Double calorie){
		...
		try {
			...
			while(res.next()) {
				try {
					list.add(new Adiacenza(res.getString("pr1"),res.getString("pr2"),res.getInt("peso")));
				} catch (Throwable t) {
					t.printStackTrace();
				}
			}
